<?php
	...
	if (gcms::isReferer() && preg_match('/^widget_([0-9]+)_([0-9]+)_([0-9]+)_([0-9]+)_(list|icon|thumb)$/', $_POST['id'], $match)) {
		// อ่านโมดูล
		$sql = "SELECT M.`id`,M.`config`,M.`module`,D.`relate`,Q.`id`";
		...
		if ($index && $index['relate'] != '') {
			...
			$sql1 .= " WHERE Q.`module_id` = 5 AND Q.`published` = '1' AND Q.`index` = '0' AND Q.`id`>".$index['id']." AND (".implode(' OR ', $qs).") ORDER BY Q.`create_date` ASC";
			...
			$sql2 .= " WHERE Q.`module_id` = 5 AND Q.`published` = '1' AND Q.`index` = '0' AND Q.`id`<".$index['id']." AND (".implode(' OR ', $qs).") ORDER BY Q.`create_date` DESC";
