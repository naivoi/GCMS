<?php
	...
	if (gcms::isReferer() && gcms::isAdmin()) {
		$module = gcms::getVars($_POST, 'module', '');
		if (preg_match('/^(modules|widgets)\/([a-z]+)$/', $module) && is_file(ROOT_PATH."$module/admin_install.php") && (empty($_SESSION['login']['account']) || $_SESSION['login']['account'] != 'demo')) {
			$content = array();
