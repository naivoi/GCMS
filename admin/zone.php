<?php
	...
	if (MAIN_INIT == 'admin' && $isAdmin) {
		...
		$language = sizeof($language) == 0 ? array('id' => 0, 'type' => 'array') : $language[0];
		$save = array();
		...
		foreach ($save AS $key => $value) {
			...
			foreach ($config['languages'] AS $l) {
				$row .= '<td data-text='.$l.'><label class=g-input><input type=text name=language_'.$l.'[] value="'.(empty($value[$l]) ? '' : $value[$l]).'" style="background-image:url('.DATA_URL.'/language/'.$l.'.gif)" title="{LNG_COUNTRY_ZONE} '.$l.'"></label></td>';
			}
		...
		if (isset($_GET['id'])) {
			$content[] = '<input type=hidden name=next value="module=countrywrite&amp;id='.$_GET['id'].'">';
		}
		$content[] = '</div>';
		$content[] = '</form>';
		$content[] = '</section>';
