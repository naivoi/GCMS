<?php
	...
	if (gcms::isReferer() && gcms::isMember()) {
		if (isset($_SESSION['login']['account']) && $_SESSION['login']['account'] == 'demo') {
			...
			$save['company'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_company', ''));
			$save['pname'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_pname', ''));
			$save['fname'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_fname', ''));
			$save['lname'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_lname', ''));
			$save['address1'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_address1', ''));
			$save['address2'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_address2', ''));
			$save['provinceID'] = $db->sql_trim_str($_POST['register_provinceID']);
			$save['province'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_province', ''));
			$save['zipcode'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_zipcode', ''));
			$save['country'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_country', ''));
			$save['phone1'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_phone1', ''));
			$save['phone2'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_phone2', ''));
			$save['birthday'] = $db->sql_trim_str(gcms::getVars($_POST, 'register_birthday', ''));
			...
			$id = gcms::getVars($_POST, 'register_id', 0);
			...
			if ($id > 0 && !$user) {
				...
				if (isset($_POST['register_website'])) {
					$save['website'] = trim(gcms::getVars($_POST, 'register_website', ''));
					if ($save['website'] != '') {
				...
				if (!$error) {
					...
					if ($_SESSION['login']['id'] == $user['id'] && !empty($save['password'])) {
