	...
	if (gcms::isReferer() && gcms::isAdmin()) {
		if (isset($_SESSION['login']['account']) && $_SESSION['login']['account'] == 'demo') {
			...
			if ($id > 0 && !$email) {
				...
				// ค่าที่ส่งมา
				$save['language'] = $db->sql_trim_str(gcms::getVars($_POST, 'email_language', ''));
				$save['from_email'] = $db->sql_trim_str(gcms::getVars($_POST, 'email_from_email', ''));
				$save['subject'] = $db->sql_trim_str(gcms::getVars($_POST, 'email_subject', ''));
