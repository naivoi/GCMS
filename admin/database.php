<?php
	...
	if (MAIN_INIT == 'admin' && $isAdmin) {
		...
		$design_mode = isset($design_mode) && $design_mode == 'design';
		// ตารางทั้งหมด
		...
		foreach ($tables AS $table) {
			if (preg_match('/^'.PREFIX.'(.*?)$/', $table['Name'], $match)) {
				...
				if ($table['Name'] == DB_LANGUAGE) {
					...
					$tr .= '<td class=nowrap>'.implode(', ', $trs).'</td>';
					$trs = array();
					...
					$tr .= '<td><label class=nowrap><input type=checkbox name='.$table['Name'].'[] value=datas checked>&nbsp;{LNG_DATAS}</label></td>';
					$tr .= '<td>&nbsp;</td>';
				}
				$tr .= '</tr>';
				$content[] = $tr;
