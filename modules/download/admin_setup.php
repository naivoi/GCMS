<?php
	...
	if (MAIN_INIT == 'admin' && gcms::canConfig($config['download_can_upload'])) {
		...
		if (sizeof($index) == 0) {
			...
			// รายการต่อหน้า
			$list_per_page = gcms::getVars('GET,COOKIE', 'count,download_listperpage', 30);
			...
			$content[] = 'inintCheck("tbl_list");';
			$content[] = 'inintTR("tbl_list", /M_[0-9]+/);';
			$content[] = 'callAction("btn_action", function(){return $E("sel_action").value}, "tbl_list", "'.WEB_URL.'/modules/download/admin_action.php");';
			$content[] = '});';
			$content[] = '</script>';
			// หน้านี้
