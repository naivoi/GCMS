<?php
	...
	if (gcms::isReferer() && gcms::canConfig($config['personnel_can_write'])) {
		if (isset($_SESSION['login']['account']) && $_SESSION['login']['account'] == 'demo') {
			...
			// ค่าที่ส่งมา
			$save = array();
			$save['name'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_name', ''));
			$save['email'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_email', ''));
			$save['position'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_position', ''));
			$save['phone'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_phone', ''));
			$save['address'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_address', ''));
			$save['detail'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_detail', ''));
