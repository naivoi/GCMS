			...
			foreach ($categories AS $id => $items) {
				...
				foreach ($items AS $l => $item) {
					...
					if ($i == 0) {
						...
						$tr .= '<td headers="b'.$id.' c5" class="menu tablet" rowspan='.$c.'><span class="icon-reply reply'.$item['can_reply'].'" title="'.$lng['LNG_CAN_REPLIES'][$item['can_reply']].'"></span></td>';
						$tr .= '<td headers="b'.$id.' c6" class="menu tablet" rowspan='.$c.'><span class=icon-published'.$item['published'].' title="'.$lng['LNG_PUBLISHEDS'][$item['published']].'"></span></td>';
					}
