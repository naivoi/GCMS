<?php
	...
	if (gcms::isReferer() && gcms::isAdmin()) {
		if (isset($_SESSION['login']['account']) && $_SESSION['login']['account'] == 'demo') {
			...
			// ค่าที่ส่งมา
			$save['menu_text'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_menu_text', ''));
			$save['menu_tooltip'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_menu_tooltip', ''));
			$save['accesskey'] = strtolower($db->sql_trim_str($_POST['write_accesskey']));
			$save['alias'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_alias', ''));
			$save['parent'] = strtoupper($db->sql_trim_str($_POST['write_parent']));
			$save['menu_url'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_menu_url', ''));
			$save['menu_target'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_target', ''));
			$save['language'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_language', ''));
