	...
	if (gcms::isReferer() && gcms::isAdmin()) {
		if (isset($_SESSION['login']['account']) && $_SESSION['login']['account'] == 'demo') {
			...
			if (($id > 0 && !$index) || !preg_match('/^[a-z]+$/', $owner) || !is_dir(ROOT_PATH."modules/$owner")) {
				...
				// ค่าที่ส่งมา
				$language = $db->sql_trim_str(gcms::getVars($_POST, 'write_language', ''));
				$module = empty($_POST['write_module']) ? '' : strtolower($db->sql_trim_str($_POST['write_module']));
				$detail_save['topic'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_topic', ''));
				...
				$index_save['published_date'] = $db->sql_trim_str(gcms::getVars($_POST, 'write_published_date', ''));
