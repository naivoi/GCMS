	...
	if (gcms::isReferer()) {
		if (isset($_SESSION['login']['account']) && $_SESSION['login']['account'] == 'demo') {
			...
			if ($board_id > 0) {
				...
				$sql .= " LEFT JOIN `".DB_CATEGORY."` AS C ON C.`module_id`=Q.`module_id` AND C.`category_id`=$category_id";
				...
				$sql .= " LEFT JOIN `".DB_CATEGORY."` AS C ON C.`module_id`=M.`id` AND C.`category_id`=$category_id";
