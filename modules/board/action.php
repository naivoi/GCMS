<?php
	...
	if (gcms::isReferer()) {
		...
		if (preg_match('/(quote|edit|delete|pin|lock|deleting)-([0-9]+)-([0-9]+)-([0-9]+)-(.*)$/', $_POST['id'], $match)) {
			...
			$action = gcms::getVars($_POST, 'senddelete_action', '');
			$qid = gcms::getVars($_POST, 'senddelete_qid', 0);
			$rid = gcms::getVars($_POST, 'senddelete_rid', 0);
			$no = gcms::getVars($_POST, 'senddelete_no', 0);
			$module = gcms::getVars($_POST, 'senddelete_module', '');
		}
		if ($rid > 0) {
			// คำตอบ
