<?php
	...
	if (MAIN_INIT == 'admin' && gcms::canConfig($config, 'gallery_can_write')) {
		...
		if (sizeof($index) == 0) {
			...
			$count = $count[0];
			// รายการต่อหน้า
			$list_per_page = gcms::getVars('GET,COOKIE', 'count,gallery_listperpage', 30);
			...
			// คำนวณรายการที่แสดง
			$s = $start < 0 ? 0 : $start + 1;
			$e = min($count['count'], $s + $list_per_page - 1);
			$patt2 = array('/{COUNT}/', '/{PAGE}/', '/{TOTALPAGE}/', '/{START}/', '/{END}/');
			$replace2 = array($count['count'], $page, $totalpage, $s, $e);
			// title
			...
			$content[] = '<a class="button add" href="index.php?module=gallery-write&amp;src=gallery-album"><span class=icon-plus>{LNG_ADD_NEW} {LNG_GALLERY_ALBUM}</span></a>';
			$content[] = '</div>';
			$content[] = '</section>';
			$content[] = '<script>';
