<?php
	...
	if (MAIN_INIT == 'admin' && gcms::canConfig($config['gallery_can_config'])) {
		...
		if (sizeof($index) == 0) {
			...
			// การแสดงผล
			$content[] = '<fieldset>';
			$content[] = '<legend><span>{LNG_DISPLAY} {LNG_ALBUM}</span></legend>';
			...
			foreach ($config['member_status'] AS $i => $item) {
				if ($i > 1) {
					...
					$tr .= '<th>'.$item.'</th>';
					// can_write
					$tr .= '<td><label data-text="{LNG_UPLOADING}"><input type=checkbox name=config_can_write[]'.(isset($config['gallery_can_write']) && in_array($i, $config['gallery_can_write']) ? ' checked' : '').' value='.$i.' title="{LNG_CAN_UPLOAD_COMMENT}"></label></td>';
					// can_config
					$tr .= '<td><label data-text="{LNG_CAN_CONFIG}"><input type=checkbox name=config_can_config[]'.(isset($config['gallery_can_config']) && in_array($i, $config['gallery_can_config']) ? ' checked' : '').' value='.$i.' title="{LNG_CAN_CONFIG_COMMENT}"></label></td>';
					$tr .= '</tr>';
					$content[] = $tr;
				}
			}
